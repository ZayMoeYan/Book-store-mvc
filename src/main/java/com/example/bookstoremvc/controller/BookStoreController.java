...
public class BookStoreController {
    ...
    public ModelAndView listBestSellers(@RequestParam("status")String status){
        ...
        if (books.isEmpty()){
            return new ModelAndView("listbooks","books", Collections.emptyList());
        }else {
    ...
    public String landingPage(Model model){
        ...
        Set<Book> promotions=getBooksByBookStatus(BookStatus.PROMOTION)
                .stream()
                        .limit(3)
                                .collect(Collectors.toSet());
        Set<Book> recommends = getBooksByBookStatus(BookStatus.RECOMMENDED)
                .stream()
                        .limit(3)
                                .collect(Collectors.toSet());
        ...
        model.addAttribute("recommendeds",recommends);
